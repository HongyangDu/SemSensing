...
def model_load(in_c, out_c):
    model = UNet_noskip(in_c, out_c, bilinear=False).cuda()
...
def DIP_denoiser(truth_tensor, net_input, ref_truth, Phi_tensor, y_tensor, model, optimizer, loss_fn, iter_num, mu, rho, shift_step):
    loss_min = torch.tensor([100]).cuda().float()
    for i in range(iter_num):
        model_out = model(net_input)
        optimizer.zero_grad()
        x_loss = loss_fn(model_out, ref_truth) 
        outshift = shift_torch(model_out, shift_step)
        y_hat = A_torch(outshift, Phi_tensor)
        y_loss = loss_fn(y_hat, y_tensor) *rho/mu
        loss = x_loss + y_loss
        loss.backward()
        optimizer.step()
        #out = shift_back_torch(outshift, shift_step)
